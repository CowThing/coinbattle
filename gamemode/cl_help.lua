...
function GM:ShowHelp()
	...
	if ( !IsValid( Help ) ) then
		...
		Help.lblFooterText.Think = function( panel ) 
			...
			if( GAMEMODE.RoundBased && CurTime() > GAMEMODE:GetTimeLimit() ) or ( GAMEMODE.RoundLimit <= GetGlobalInt( "RoundNumber", 0 ) ) then
				panel:SetText( "Game will end after this round" ) return
			end
